...
interface LogEntry { node: string; [k: string]: unknown }
...
const pickSubtask = (state: typeof LessonState.State) => {
  if (state.done) return {};
  const topic = state.todo[0] as TopicNode | undefined;
  if (!topic) {
    return { done: true, logs: [{ node: 'pickSubtask', action: 'no_topics' }] };
  }
  const remaining = topic.subtasks.filter((s: { id: string }) => !state.lessons.find(l => (l as any).subtaskId === s.id));
  if (!remaining.length) {
...
const checkQuestions = (state: typeof LessonState.State) => {
  // Espera que fila esteja anexada externamente no state (lessonManager estende)
  const qs: QuestionState[] = (state.questionsQueue || []);
  const chatNow = qs.find(q => q.route === 'CHAT_NOW');
  if (chatNow) {
    return { logs: [{ node: 'checkQuestions', action: 'answer_now', q: chatNow.id }], currentQuestionId: chatNow.id };
  }
  return { logs: [{ node: 'checkQuestions', action: 'none' }] };
};

// Responder pergunta rápida inline (placeholder: eco curto)
const answerChatNow = async (state: typeof LessonState.State) => {
  const qid = state.currentQuestionId;
  if (!qid) return {};
  const q = state.questionsQueue.find(q => q.id === qid);
  if (!q) {
  return { currentQuestionId: null, logs: [{ node: 'answerChatNow', action: 'missing_question', qid }] };
  }
  let answer = '';
  try {
    const full = await ragAgent.answerWithCitations(q.text, state.currentTopicId || undefined);
    // Para respostas rápidas, reduzir se não marcar needsRAG
    answer = q.needsRAG ? full : full.split(/\n+/).slice(0, 4).join('\n');
  } catch (e) {
    answer = 'Erro ao responder agora: ' + (e as Error).message;
  }
  const newAnswers = [...state.answers, { id: 'ans_' + qid, questionId: qid, answer, mode: 'chat_now', ts: Date.now() }];
  const remainingQuestions = state.questionsQueue.filter(x => x.id !== qid);
  return {
    answers: newAnswers,
    questionsQueue: remainingQuestions,
...
const processPauseAnswers = async (state: typeof LessonState.State) => {
...
// Respostas de fim de tópico - somente quando último subtask concluído
const processEndTopicAnswers = async (state: typeof LessonState.State) => {
  // Detectar fim de tópico: currentSubtaskId já null e próximo pick irá avançar se não restam subtasks
  const topic = state.todo[0] as TopicNode | undefined;
  if (!topic) return { logs: [{ node: 'processEndTopicAnswers', action: 'skip_no_topic' }] };
  const remaining = topic.subtasks.filter(s => !state.lessons.find(l => (l as any).subtaskId === s.id));
  const isEnd = remaining.length === 0; // topic finalizado
  const items = (state.broadcastQueue || []).filter(i => i.route === 'END_TOPIC');
  if (!isEnd || !items.length) return { logs: [{ node: 'processEndTopicAnswers', action: 'skip' }] };
  const newAnswers = [...state.answers];
  for (const item of items) {
    const q = state.questionsQueue.find(q => q.id === item.questionId) || { id: item.questionId, text: item.text } as QuestionState;
    try {
      const full = await ragAgent.answerWithCitations(q.text, topic.id);
      newAnswers.push({ id: 'ans_' + q.id, questionId: q.id, answer: full, mode: 'end_topic', ts: Date.now() });
    } catch (e) {
      newAnswers.push({ id: 'ans_' + q.id, questionId: q.id, answer: 'Erro fim de tópico: ' + (e as Error).message, mode: 'end_topic', ts: Date.now() });
    }
  }
  const answeredIds = new Set(items.map(i => i.questionId));
  return {
    answers: newAnswers,
    questionsQueue: state.questionsQueue.filter(q => !answeredIds.has(q.id)),
    broadcastQueue: (state.broadcastQueue || []).filter(i => i.route !== 'END_TOPIC'),
    logs: [{ node: 'processEndTopicAnswers', count: items.length }]
  };
};

const draftLesson = async (state: typeof LessonState.State) => {
...
const groundWithRag = async (state: typeof LessonState.State) => {
  if (!state.currentTopicId || !state.currentSubtaskId) return {};
  const topic = state.todo.find(t => (t as TopicNode).id === state.currentTopicId) as TopicNode | undefined;
  const sub = topic?.subtasks.find((s: { id: string }) => s.id === state.currentSubtaskId);
  if (!topic || !sub) return {};
  ...
  return { logs: [{ node: 'groundWithRag', action: 'llm_generate', topic: (topic as any).id, subtask: sub.id }], grounded: section.draft };
};

const finalizeSection = (state: typeof LessonState.State) => {
...
const markDoneIfFinished = (state: typeof LessonState.State) => {
...
export function buildLessonGraph() {
  ...
  g.addNode('pick', pickSubtask);
  g.addNode('checkQuestions', checkQuestions);
  g.addNode('answerChatNow', answerChatNow);
  g.addNode('processPauseAnswers', processPauseAnswers);
  g.addNode('processEndTopicAnswers', processEndTopicAnswers);
  g.addNode('draftNode', draftLesson);
  g.addNode('groundNode', groundWithRag);
  g.addNode('finalizeSection', finalizeSection);
  g.addNode('checkDone', markDoneIfFinished);
